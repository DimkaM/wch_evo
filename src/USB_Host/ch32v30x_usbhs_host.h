...
extern void USBHSH_ResetRootHubPortSpeed( uint8_t mode, uint8_t speed );
...
/* SPLIT transaction support (device behind a HUB on a high-speed link) */
extern void USBHSH_SplitWaitMicroframe( void );
extern uint8_t USBHSH_HubSplitTransact( uint16_t split_data, uint8_t endp_pid, uint8_t endp_tog, uint8_t data_no, uint32_t timeout );
