...
/*********************************************************************
 * @fn      USBHSH_ResetRootHubPortSpeed
 *
 * @brief   Reset USB port with the specified expected port speed.
 *
 * @para    mode : Reset host port operating mode.
 *                 0 -> reset and wait end
 *                 1 -> begin reset
 *                 2 -> end reset
 *          speed: Expected speed of the root port (USB_HIGH_SPEED / USB_FULL_SPEED /
 *                 USB_LOW_SPEED). It is set before the bus reset is issued, because the
 *                 high-speed handshake is performed during the reset: with USB_FULL_SPEED
 *                 no handshake is performed and a high-speed HUB operates at full speed,
 *                 so the devices behind it need no SPLIT transactions.
 *
 * @return  none
 */
void USBHSH_ResetRootHubPortSpeed( uint8_t mode, uint8_t speed )
{
 ...
 *
 * @brief   Reset USB port with the default (high-speed) expected speed.
 *
...
{
    USBHSH_ResetRootHubPortSpeed( mode, USB_HIGH_SPEED );
}
...
/*********************************************************************
 * @fn      USBHSH_SplitWaitMicroframe
 *
 * @brief   Wait for the next microframe boundary. The SPLIT transaction phases (start split and
 *          complete split) have to be separated by at least one microframe.
 *
 * @return  none
 */
void USBHSH_SplitWaitMicroframe( void )
{
    uint16_t mf = USBHSH->FRAME_NO & USBHS_MICRO_FRAME_NUM;
    uint32_t timeout = 200000;

    while( ( ( USBHSH->FRAME_NO & USBHS_MICRO_FRAME_NUM ) == mf ) && ( timeout != 0 ) )
    {
        timeout--;
    }
}
...
/*********************************************************************
 * @fn      USBHSH_HubSplitTransact
 *
 * @brief   Perform one phase of a SPLIT transaction to a device connected behind a HUB. The
 *          high-speed link to the HUB is kept, the HUB address, the HUB port number and the
 *          phase of the split are transferred inside R16_UH_SPLIT_DATA, and the "no data"
 *          phase is selected with RB_UH_R_DATA_NO / RB_UH_T_DATA_NO.
 *
 * @para    split_data: content of the SPLIT packet (R16_UH_SPLIT_DATA, 12 bits are valid)
 *          endp_pid  : token PID and endpoint number
 *          endp_tog  : data toggle to be prepared
 *          data_no   : 0 - phase with data, 1 - phase without data
 *          timeout   : transfer timeout, in units of the polling loop iteration
 *
 * @return  0xFF if the transfer did not complete, otherwise the content of R8_USB_INT_ST
 */
uint8_t USBHSH_HubSplitTransact( uint16_t split_data, uint8_t endp_pid, uint8_t endp_tog, uint8_t data_no, uint32_t timeout )
{
    uint32_t i;
    uint8_t  res;

    USBHSH->HOST_SPLIT_DATA = split_data & USBHS_UH_SPLIT_DATA;
    USBHSH->HOST_RX_CTRL = endp_tog | ( data_no ? USBHS_UH_R_DATA_NO : 0x00 );
    USBHSH->HOST_TX_CTRL = endp_tog | ( data_no ? USBHS_UH_T_DATA_NO : 0x00 );

    USBHSH->HOST_EP_PID = endp_pid;             /* Specify token PID and endpoint number */
    USBHSH->INT_FG = USBHS_UIF_TRANSFER;        /* Allow transfer */
    for( i = timeout; ( i != 0 ) && ( ( USBHSH->INT_FG & USBHS_UIF_TRANSFER ) == 0 ); i-- )
    {
        Delay_Us( 1 );
    }
    USBHSH->HOST_EP_PID = 0x00;                 /* Stop USB transfer */
    ...
    if( ( USBHSH->INT_FG & USBHS_UIF_TRANSFER ) == 0 )
    {
        return 0xFF;                            /* The transaction did not complete */
    }

    res = (uint8_t)USBHSH->INT_ST;
    return res;
}
